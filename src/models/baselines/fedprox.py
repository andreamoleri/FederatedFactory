...
class FedProxBaseline(FederatedBaseline):
    ...
    def _effective_lr(self, round_num: int) -> Tuple[float, float, float]:
        """
        Calculate the effective learning rate for the current round using exponential decay.

        Formula:
        $$ LR_t = LR_{base} \times (decay)^{round\_num} $$

        Args:
            round_num (int): The current training round (0-based index).
        ...
        # Enforce logical bounds on the decay factor.
        if round_decay > 1.0:
            logger.warning(
                "[FedProx] baseline_round_lr_decay=%.4f > 1.0; "
        ...
        if round_decay < 0.0:
            logger.warning(
                "[FedProx] baseline_round_lr_decay=%.4f < 0.0; "
                "clamping to 0.0.",
                round_decay,
            )
            round_decay = 0.0
