...
# ORGANIZATION
# ==============================================================================
LOG_WORK_DIR="$SCRIPT_DIR/run_federated_baselines_gpu2"
...
# 1. FedAvg (Grid: Learning Rate)

for lr in "${LRS[@]}"; do
    MODELS+=("baseline:fedavg|--learning-rate $lr")
done


# 2. FedProx (Grid: Learning Rate x Mu) "0.1" "0.01" "0.001"
PROX_MUS=("0.1" "0.01")
for lr in "${LRS[@]}"; do
    for mu in "${PROX_MUS[@]}"; do
        MODELS+=("baseline:fedprox|--learning-rate $lr --fedprox-mu $mu")
    done
done

# 3. FedDyn (Grid: Learning Rate x Alpha) "0.1" "0.01" "0.001"
DYN_ALPHAS=("0.01" "0.001" "0.0001")
for lr in "${LRS[@]}"; do
    for alpha in "${DYN_ALPHAS[@]}"; do
        MODELS+=("baseline:feddyn|--learning-rate $lr --feddyn-alpha $alpha")
    done
done


# 4. Scaffold (Grid: Learning Rate x Momentum)
MOMS=("0.0" "0.9")
for lr in "${LRS[@]}"; do
    for mom in "${MOMS[@]}"; do
        MODELS+=("baseline:scaffold|--learning-rate $lr --baseline-momentum $mom")
    done
done
...
for SEED in "${SEEDS[@]}"; do
  for DATASET in "${DATASETS[@]}"; do
    ...
    for MODEL_ENTRY in "${MODELS[@]}"; do
      ...
      # [NEW] Create a unique suffix for the folder based on args
      # e.g., converts "--learning-rate 0.1" to "_learning_rate_0.1"
      ...
      for EPS in "${EPOCHS_LIST[@]}"; do
        ...
        for ALPHA in "${ALPHAS[@]}"; do
            # [EDITED] Added ${ARG_SUFFIX} to path
            OUT_DIR="$PROJECT_ROOT/output/main_experiments_gpu2/baselines_dirichlet/epochs_${EPS}/${SAFE_DATASET_NAME}/${MODEL}${ARG_SUFFIX}/alpha_${ALPHA}/seed_${SEED}"

            # Log naming (Logs are safe because of JOB_COUNT, but adding suffix helps readability)
        ...
        if [ "$HAS_NATURAL_SPLIT" = true ]; then
            # [EDITED] Added ${ARG_SUFFIX} to path
            OUT_DIR="$PROJECT_ROOT/output/main_experiments_gpu2/baselines_silos/epochs_${EPS}/${SAFE_DATASET_NAME}/${MODEL}${ARG_SUFFIX}/seed_${SEED}"
